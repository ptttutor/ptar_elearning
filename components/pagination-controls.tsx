...
export function PaginationControls({ currentPage, totalPages, onPageChange }: { currentPage: number; totalPages: number; onPageChange: (page: number) => void }) {
  ...
  const navButtonClass = "flex h-9 w-9 shrink-0 items-center justify-center rounded-full p-0 hover:bg-primary/10 hover:border-primary"
  ...
  return (
    <>
      ...
      {pageList.map((page, idx) =>
        page === "..." ? (
          <span key={`dots-${idx}`} className="flex h-9 w-9 shrink-0 items-center justify-center text-muted-foreground">
            &#8230;
          </span>
        ) : (
          <Button
            ...
            className={`flex h-9 min-w-[2.5rem] shrink-0 items-center justify-center rounded-full px-0 ${
              currentPage === page ? "bg-primary hover:bg-primary/90 text-primary-foreground" : "hover:bg-primary/10 hover:border-primary"
            }`}
          >
            {page}
          </Button>
