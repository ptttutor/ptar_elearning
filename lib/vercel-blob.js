...
async function compressImageIfNeeded(file, maxSizeBytes = 2 * 1024 * 1024, quality = 0.8) {
  ...
  // GIFs are skipped entirely — canvas drawImage() only captures a single
  // frame, so compressing an animated GIF would silently flatten it.
  if (!file.type.startsWith('image/') || file.type === 'image/gif' || file.size <= maxSizeBytes) {
    return file;
  }
