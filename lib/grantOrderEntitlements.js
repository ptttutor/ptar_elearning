...
export async function grantEntitlementsForOrder(orderId) {
  ...
  for (const item of order.items) {
    if (item.itemType === "COURSE") {
      ...
      const exists = await prisma.flashcardDeckPurchase.findUnique({
        where: { userId_deckId: { userId: order.userId, deckId: item.itemId } },
      });
      if (!exists) {
        await prisma.flashcardDeckPurchase.create({
          data: { userId: order.userId, deckId: item.itemId, orderId: order.id },
        });
      }
    }
