...
export async function fetchBookReviews(ebookId: string, page: number, limit: number): Promise<{ reviews: ApiReview[]; stats?: ReviewsStats; hasMore: boolean }> {
  const url = `/api/reviews?ebookId=${encodeURIComponent(ebookId)}&page=${page}&limit=${limit}`
...
export async function postBookReview(payload: { userId: string; ebookId: string; rating: number; title: string; comment: string }): Promise<ApiReview | null> {
